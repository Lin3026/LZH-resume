...
function EducationRow({ item, index }: { item: EducationItem; index: number }) {
  return (
    <FadeIn delay={index * 0.12} y={30} className="relative pl-12 sm:pl-16">
      {/* 圆点 */}
      <span
        className="absolute left-[14px] sm:left-[22px] top-2 w-3.5 h-3.5 rounded-full bg-[#1a1a1a] ring-4 ring-white"
        aria-hidden="true"
      />
      {/* 日期 */}
      <span className="block text-xs sm:text-sm uppercase tracking-widest text-[#1a1a1a]/55 mb-2">
        {item.period}
      </span>
      {/* 学校名 */}
      <span
        className="block font-black leading-tight text-[#1a1a1a] mb-2"
      ...
      {/* 标签行 */}
      <span
        className="block font-medium text-[#1a1a1a]/80 mb-4"
      ...
      {/* 详情 */}
      <p
        className="whitespace-pre-line rounded-[24px] sm:rounded-[28px] border border-[#1a1a1a]/10 bg-[#fafafa] p-5 sm:p-6 leading-relaxed text-[#1a1a1a]/75"
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-3xl">
        {/* 竖线 */}
        <span
          className="absolute left-[21px] sm:left-[29px] top-2 bottom-2 w-px bg-[#1a1a1a]/15"
          aria-hidden="true"
        />
