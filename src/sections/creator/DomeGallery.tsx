...
/* ============================================================
 * DomeGallery — 3D 球形媒体画廊（基于 React Bits）
 * 改造点：原组件只支持图片，这里扩展为同时支持「视频」。
...
type MediaInput = string | { src: string; alt?: string; type?: 'image' | 'video' };
...
function buildItems(pool: MediaInput[], seg: number): BuiltItem[] {
  ...
  // 归一化：区分 image / video
  const normalized = pool.map((media): { src: string; alt: string; type: 'image' | 'video' } => {
    if (typeof media === 'string') {
      return { src: media, alt: '', type: 'image' };
    }
    return { src: media.src || '', alt: media.alt || '', type: media.type === 'video' ? 'video' : 'image' };
  });
...
export default function DomeGallery({
  ...
  // 轻微自转：缓慢绕 Y 轴旋转，拖动 / 惯性 / 放大预览时暂停
  useEffect(() => {
    let raf = 0;
    let last = performance.now();
    const SPEED_DEG_PER_SEC = 5; // 轻微自转速度
    const tick = (now: number) => {
      ...
      if (!paused) {
        const delta = SPEED_DEG_PER_SEC * (dt / 1000);
  ...
  const openItemFromElement = useCallback(
    (el: HTMLElement) => {
      ...
      // ===== 视频 / 图片分支 =====
      const type = (parent.dataset.type as 'image' | 'video') || 'image';
      ...
      if (type === 'video') {
        ...
        const ICON = {
          ...
          mute: '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M3 9v6h4l5 5V4L7 9H3z"/><path d="M16 9l4 4m0-4l-4 4" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round"/></svg>',
          sound: '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M3 9v6h4l5 5V4L7 9H3z"/><path d="M16 8a4 4 0 010 8" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round"/></svg>',
        };

        const bar = document.createElement('div');
        bar.className = 'dg-video-controls';

        const playBtn = document.createElement('button');
        playBtn.type = 'button';
        playBtn.className = 'dg-ctrl-btn';
        playBtn.setAttribute('aria-label', '播放或暂停');

        const muteBtn = document.createElement('button');
        muteBtn.type = 'button';
        muteBtn.className = 'dg-ctrl-btn';
        muteBtn.setAttribute('aria-label', '开启或关闭声音');

        const syncPlay = () => {
          playBtn.innerHTML = v.paused ? ICON.play : ICON.pause;
        };
        const syncMute = () => {
          muteBtn.innerHTML = v.muted ? ICON.mute : ICON.sound;
        };
            ...
            {items.map((it, i) => (
              ...
              >
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  aria-label={it.alt || '打开媒体'}
                  onClick={onTileClick}
                  onPointerUp={onTilePointerUp}
                >
                  {it.type === 'video' ? (
                    <video src={it.src} muted playsInline preload="metadata" draggable={false} aria-hidden="true" />
                  ) : (
