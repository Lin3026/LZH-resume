      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredSkills.map((skill) => (
            <div
              key={skill.name}
              className="bg-slate-800 border border-slate-700 rounded-xl p-4 hover:border-cyan-500/50 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-cyan-500/10"
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-cyan-400">{levelLabels[skill.level]}</div>
                </div>
              </div>
              {/* 进度条 */}
                ...
                {[1, 2, 3, 4, 5].map((dot) => (
                  <div
                    ...
                    className={`w-1.5 h-1.5 rounded-full transition-all ${
                      dot <= skill.level ? 'bg-cyan-400' : 'bg-slate-700'
                    }`}
                  />
                ))}
              </div>
