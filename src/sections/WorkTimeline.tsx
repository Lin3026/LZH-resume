    ...
    <section id="experience" className="py-20 relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-6 md:px-12">
        <div className="relative pl-16 md:pl-24">
          {/* 时间轴线 - 虚线样式 */}
          <div
            className="absolute left-6 md:left-12 top-0 bottom-0 w-[2px] border-l-[3px] border-dashed border-cyan-400/60"
            ...
            {workExperiences.map((exp, index) => {
              ...
              return (
                <div key={exp.id} className="relative flex items-stretch group">
                  {/* 时间轴节点圆圈 */}
                  <div className="absolute left-[-(md:px-12)] left-[-2rem] md:left-[-3rem] w-6 h-6 mt-7 rounded-full border-[3px] border-cyan-400 bg-cyan-900/80 shadow-[0_0_10px_rgba(34,211,238,0.5)] flex-shrink-0 z-10" style={{
                    left: index === 0 ? '-2rem' : undefined,
                  }}>
                    <div className="absolute inset-1 rounded-full bg-cyan-300/50" />
                  </div>

                  {/* 年份标签 - 在时间线左侧 */}
                  <div className="absolute left-[-(md:pl-12)] hidden md:flex items-center justify-end pr-4" style={{
                    width: 'calc(3rem - 12px)',
                    left: 0,
                    transform: 'translateX(-100%)',
                    marginTop: '1.75rem',
                  }}>
                    <span className="text-3xl md:text-4xl font-bold font-mono text-amber-300/90 select-none whitespace-nowrap drop-shadow-[0_2px_8px_rgba(0,0,0,0.5)]">
                      {year}年
                    </span>
                  </div>

                  {/* 移动端年份 */}
                  <div className="md:hidden absolute -left-16 -top-2">
                    <span className="text-lg font-bold font-mono text-amber-300/90">{year}</span>
                  </div>
                    ...
                    <div className="bg-white/80 border-2 border-blue-200/60 rounded-2xl p-5 md:p-6 shadow-xl hover:shadow-2xl transition-all duration-300 hover:border-cyan-400/70 ocean-card group-hover:-translate-y-0.5">
                      <CardContent className="p-0 flex gap-4 items-start">
                        ...
                        <div className="flex-1 min-w-0">
                          {/* 岗位 / 公司名称 */}
                          <h3 className="text-xl md:text-2xl font-bold text-blue-900 mb-2 leading-tight">
                            {exp.position} <span className="text-cyan-500">/</span> {exp.company}
                          </h3>
                          ...
                          {/* 描述文字 - 直接展示，不需要点击展开 */}
                          <p className="text-blue-800/85 text-sm md:text-base leading-relaxed whitespace-pre-line">
                            {exp.description}
                          </p>
                        </div>
                      </CardContent>
